...
import { analyzeMidiFile } from '../services/midiAnalysisService';
...
interface FileUploadCardProps {
  ...
  onMidiLearning?: (description: string) => void;
  analysisResult: AnalysisResult | null;
  apiConfig: ApiConfig;
}

const FileUploadCard: React.FC<FileUploadCardProps> = ({ onAnalysisComplete, onMidiLearning, analysisResult, apiConfig }) => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      if (isMidiFile) {
        console.log('Processing MIDI file:', file.name);
        // Handle MIDI file analysis and learning
        const midiResult = await analyzeMidiFile(file, apiConfig);
        console.log('MIDI analysis result:', midiResult);
        
        // Trigger learning from MIDI description
        if (onMidiLearning) {
          console.log('Triggering MIDI learning with description:', midiResult.description);
          onMidiLearning(midiResult.description);
          setSuccessMessage('MIDI file analyzed and learned from successfully!');
          setTimeout(() => setSuccessMessage(null), 3000);
        }
        
        // Convert MIDI analysis to AnalysisResult format for display
        const analysisResult: AnalysisResult = {
          bpm: midiResult.bpm,
          key: midiResult.key,
          mood: midiResult.mood,
          genre: midiResult.genre,
          chords: [], // MIDI files don't have explicit chord progressions
          instruments: midiResult.instruments,
          tempoCategory: midiResult.bpm > 140 ? 'Fast' : midiResult.bpm > 120 ? 'Moderate' : 'Slow',
          loudness: -12, // Default for MIDI
          fingerprint: `midi-${Date.now()}` // Simple fingerprint for MIDI files
        };
  ...
  }, [file, onAnalysisComplete, onMidiLearning, apiConfig]);

  return (
    <div className="bg-surface border border-surface-border rounded-lg p-6 mb-6 shadow-lg">
      <h2 className="text-xl font-bold text-secondary mb-4">1. Upload & Analyze Audio/MIDI</h2>
      <div className="flex items-center space-x-4">
        <label className="flex-1 block">
          <span className="sr-only">Choose file</span>
          <input type="file" onChange={handleFileChange} accept="audio/*,.mid,.midi"
