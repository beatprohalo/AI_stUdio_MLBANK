...
const BulkImportCard: React.FC<BulkImportCardProps> = ({ onBulkLearn, isEnabled, processedFiles }) => {
  ...
  const handleImport = async (files: FileList, type: 'chords' | 'melodies' | 'humanization') => {
    ...
    const unprocessedFiles = Array.from(files).filter(file => 
      !processedFiles?.has(file.name)
    );

    if (unprocessedFiles.length === 0) {
      setSuccessMessage('All selected files have already been processed. Please choose different files.');
      setTimeout(() => setSuccessMessage(null), 3000);
      return;
    }

    if (unprocessedFiles.length < files.length) {
      const skippedCount = files.length - unprocessedFiles.length;
      setSuccessMessage(`${skippedCount} file(s) were skipped as they were already processed. Processing ${unprocessedFiles.length} new file(s)...`);
      setTimeout(() => setSuccessMessage(null), 2000);
    }

    const descriptions = unprocessedFiles.map(file => {
    ...
    setProgress(null);
    setSuccessMessage(`Successfully learned from ${unprocessedFiles.length} ${type} file(s)!`);
    setTimeout(() => setSuccessMessage(null), 3000);
  };
